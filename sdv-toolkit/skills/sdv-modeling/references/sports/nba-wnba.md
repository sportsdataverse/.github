...
- Box-derived possessions are the **default** for team ratings; the event-level `nba_possessions` engine is the escalation only if the oracle MAE floor fails, not the baseline (`poss-design.md` §7.3; `design.md` §3.3).
- **WNBA pre-2005 team-log repair** (in the `wehoop-wnba-stats-data` producer consuming this engine, not sdv-py itself): early team logs are corrupt (`min`=8.0/2000, 0.9/2002, string/1997+2003; `tov`≈0 pre-2001) while **player** logs are pristine every era — repair is player→team only: `min := player-sum`, `tov := max(team_col, player-sum)` (`ClaudeCowork/notes/2026-07-28-espn-nba-names-handoff.md:34-36`).
