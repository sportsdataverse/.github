...
| `references/feature-construction.md` | How do I get more signal out of the columns — which learner needs transforms, correlated columns splitting importance, finding and testing interactions, auto-generated features against a null bar, one model's output as another's input, missingness, and a predict-time smoke test? |
...
| `references/competition.md` | Playing a leaderboard — Kaggle or a hackathon: is every input clean and carrying real signal, does CV track the private board, and is the OOF → stack → post-process chain free of leakage? |
| `references/tracking-data-cv.md` | Working with tracking data or video — which way the court faces, which frame an event id points at, pixels to court coordinates, tracks, and kinematics? |
| `references/betting-markets.md` | Using or beating a betting market — de-vig, the market as a baseline, which line existed when, placeholder odds, CLV, and staking? |
| `references/literature.md` | What has been PUBLISHED on this family — keyed to the 717-file corpus (duplicates and empty stubs included) already on disk at `Sports-Research-Papers/md/`, plus verified external anchors. |
...
- **Combining, transforming or auto-generating features, or feeding one model's
  output into another?** → `feature-construction.md` — its §0 says whether your
  learner will benefit at all.
...
- **Is this a competition or hackathon?** → `competition.md` first — run its §1
  input signal audit before any tuning, and its §12 checklist before submitting.
- **Are the inputs clean and actually carrying signal?** → `competition.md` §1,
  whether or not there is a leaderboard.
- **Tracking data or video?** → `tracking-data-cv.md`, then `competition.md` §1.
- **Does a betting market enter the model — as a feature, a baseline, or a
  strategy?** → `betting-markets.md`, then `metrics-and-gates.md` for the gate.
