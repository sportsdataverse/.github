...
|---|---|---|---|
| ESPN | `sdv-add-espn-league` + `sdv-add-sport-parser` | `site.api.espn.com` / `sports.core.api.espn.com` | None (public) |
| Fox Sports (Bifrost) | `sdv-add-fox-league` | `api.foxsports.com/bifrost/v1` | `apikey` + `api-version` query params |
| CBS Sports (NAPI) | `sdv-add-cbs-league` | `api.cbssports.com/napi` | None for data resources |
| Yahoo Sports | `sdv-add-yahoo-source` | `sports.yahoo.com` (HTML scrape) | None (scrape) |
| 247Sports | See below | `247sports.com` | None (HTML scrape) |
| Barttorvik | See below | `barttorvik.com` | None (CSV/JSON endpoints) |

**For ESPN** → use `sdv-add-espn-league` (wrapper registration) and `sdv-add-sport-parser` (parser layer). This skill does not repeat those steps.
...
→ Follow `sdv-add-espn-league` for wrapper registration.  
→ Follow `sdv-add-sport-parser` for the parser layer.

### Fox Sports

→ Follow `sdv-add-fox-league`. The OpenAPI at `c:\Users\saiem\Documents\GitHub-Data\sdv-dev\sdv-swagger\foxsports-api-openapi.yaml` covers the full Bifrost surface.

### CBS Sports

→ Follow `sdv-add-cbs-league`. Regenerate the OpenAPI with `python cbs/gen_napi_openapi.py` after capturing new resources.

### Yahoo Sports

→ Follow `sdv-add-yahoo-source`. Extracting the embedded JSON blob is the critical first step.
