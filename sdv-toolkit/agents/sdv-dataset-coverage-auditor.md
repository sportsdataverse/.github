...
description: Use to audit how far a dataset — or the full inventory — has propagated across the 12-surface checklist `sdv-dataset-lifecycle` drives (release asset, Python loader, loader schema, returns table, reference docs, R parity loader, DB catalog row, Postgres ETL, API exposure, platform freshness, validation registration, model registry). Read-only: it reports gaps, it never edits a catalog, a loader, a schema file, or a doc — its output is a ranked backlog, not a mandate. Every surface resolves to one of three verdicts — present (verified with file:line), missing (the checkable condition failed), or unverifiable-offline (one of the checklist's own documented non-static surfaces: S1's release asset needs a live GitHub API call rather than a repo read, S3's "matches the release parquet's current columns" clause needs a network read of the parquet footer, S5's `generate.py --check` is a repo-global drift gate that can't be attributed to one dataset, S10's platform-DB-tab half needs a running site plus a live DB, and S8 splits into a static precondition and a runtime-only completion that must never be reported satisfied on the precondition's evidence alone). Invoke for "audit dataset coverage", "which surfaces is X missing", or a full-inventory sweep; dispatched by `sdv-dataset-lifecycle` as its Phase 2 starting point, and by the coverage calibration sweep.
...
`hockey-dev/fastRhockey`, the `<league>-data` family) — read them directly, do not
clone/fetch.
...
**2 — Python loader.** Fully static.
`grep -n '<DATASET>_URL' sdv-py/sportsdataverse/config.py` and
`grep -n 'def load_<dataset>' sdv-py/sportsdataverse/<league>/<league>_loaders.py`. Both
hit → `present` with both `file:line`s. Either miss → `missing`.

**3 — Loader schema.** Split. Key existence is static:
`grep -n '^load_<dataset>:' sdv-py/tools/codegen/schemas/loader_schemas.yaml`. That half
alone gives `present`/`missing` for "the key exists with *a* column list." The
...
does the dataset's `load_<dataset>` function appear in its generated reference page —
`grep -rl 'load_<dataset>' sdv-py/docs/docs/<sport>/reference/`. Use that for
...
filename search — see the table's warning). First decide applicability: does this
dataset's domain belong to one of `cfbfastR` / `hoopR` / `wehoop` / `fastRhockey` at
all? If not, `not-applicable`. If yes,
`grep -c 'export(load_<dataset>' <sibling-repo>/NAMESPACE` — nonzero → `present` (cite
the NAMESPACE line and the R file it resolves to); zero → `missing`.

**7 — Catalog row.** Fully static.
`grep -n '"<league>", "<dataset>"' sdv-db/python/src/sdv_db/catalog.py` (or read
`REGISTRY`'s construction and confirm the `(league, name)` tuple resolves).
`present`/`missing` from the grep.
...
**9 — API exposure.** Fully static.
`grep -n '<dataset>' sdv-db/python/src/sdv_db/api/gen/curation.yaml` for the source
entry, then confirm the generated `api/generated/endpoints.py` resolves an endpoint for
it (`grep -n '<dataset>' sdv-db/python/src/sdv_db/api/generated/endpoints.py`). Both
...
**11 — Validation registration.** Fully static.
`grep -n 'parquet_glob.*<dataset>' sdv-py/tools/validation/registry.py` (or read the
`DatasetSpec` list and match on the dataset's glob). `present`/`missing` from the grep.

**12 — Model registry.** First decide applicability: is this dataset a model's output
(not raw/derived pass-through data)? If not, `not-applicable`. If yes, fully static:
`ls <league>-data/docs/models/` for a lineage doc naming the dataset, and
`grep -rl 'schedule:' <league>-data/.github/workflows/` for a workflow whose schedule
trigger covers its retrain. Both present → `present`; lineage doc exists but no
`schedule:` trigger (manual-only) → `missing`, and say "retrain path exists but isn't
wired to a cron — orphan-workflow risk," not "no retrain exists."

## Modes

- **Single dataset**: given `(league, dataset)`, run all 12 rows and return the table
  below for that one dataset.
- **Full-inventory sweep**: iterate over every dataset. Default inventory source is the
  union of (a) every top-level `load_<x>:` key in
  `sdv-py/tools/codegen/schemas/loader_schemas.yaml` and (b) every `(league, name)` pair
  in `sdv-db/python/src/sdv_db/catalog.py`'s `REGISTRY` — a dataset can appear in either
  without appearing in both, and both cases are real gaps worth surfacing (a dataset
  registered in `sdv-db` with no loader schema is missing surface 3 outright; a dataset
  with a loader schema but no catalog row is missing surface 7). A caller-supplied
  dataset list overrides this default and scopes the sweep to it.
...
| 2 | Python loader | present | sportsdataverse/nfl/nfl_loaders.py:60 |
| 3 | Loader schema | present (key only) | loader_schemas.yaml:412; column-agreement unverifiable-offline — needs a live read of the release parquet footer |
| 4 | Returns table | missing | manual_column_descriptions.yaml: no entry for `xyac_median` |
...
| 7 | Catalog row | missing | no (nfl, xyac) tuple in sdv_db/catalog.py REGISTRY |
| 8 | Postgres ETL | precondition missing (surface 7 absent); completion unverifiable-offline — needs a live api.ingest_manifest query | |
| 9 | API exposure | missing | not in sdv_db/api/gen/curation.yaml |
| 10 | Platform freshness | missing (code half); unverifiable-offline (live-tab half) | not in heartbeat.py _TS_CANDIDATES |
| 11 | Validation registration | missing | no DatasetSpec in registry.py |
| 12 | Model registry | not-applicable | not a model output |

counts: present 2, missing 7, unverifiable-offline 2 (+1 partial on #3, #10),
not-applicable 2
```
